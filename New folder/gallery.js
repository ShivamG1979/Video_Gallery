...
function setupCardInteractions() {
  ...
  document.querySelectorAll(".video-item").forEach((card) => {
    ...
    // Mobile: Handle touch/tap events with passive setting
    card.addEventListener("touchstart", (e) => {
      // Skip processing if the user is scrolling
      if (isScrolling) return;
      
      // If a card was previously touched, hide its overlay
      ...
      if (card.classList.contains("card-active")) {
        // If already active, clicking the play button
        if (!e.target.closest('.play-button')) {
          // Only prevent default if event is cancelable and not already scrolling
    ...
    }, { passive: false }); // Can't be fully passive since we conditionally use preventDefault
  ...
  // Hide active card overlay when clicking elsewhere on the page (passive listener)
  document.addEventListener("touchstart", (e) => {
    if (touchedCard && !e.target.closest('.video-item')) {
